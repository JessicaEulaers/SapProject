sap.ui.define([
    ...
    return BaseController.extend("be.ap.flight.controller.Detail", {
        ...
        onPress: function (oEvent) {
            ...
            ///FlightScheduleSet(Carrid='AA',Connid='0017')/FlightSet
            var sPath = this.getModel().createKey("FlightScheduleSet", {
                Carrid: oFlightSchedule.getProperty('Carrid'),
                Connid: oFlightSchedule.getProperty('Connid')
            });
            sPath = "/" + sPath + "/FlightSet";
            ...
            this.getModel().read(sPath, {
                success: function (oData) {
                    this.getModel("detailView").setProperty("/Flights", oData.results);
                }.bind(this)
            });
        },
        onDateChange: function(oEvent){
            var oDate = oEvent.getSource().getDateValue();
            oDate = new Date(oDate.setHours(oDate.getHours()+2));
            var oTable = this.byId("Flights");
            var oBindingItems = oTable.getBinding("items");
            var aFilters=[];
            aFilters.push(new sap.ui.model.Filter({path:"Fldate",operator:"EQ",value1:oDate}));
            oBindingItems.filter(aFilters);
        },
        onBtnClearFilterClick:function(oEvent){
            var oTable = this.byId("Flights");
            var oBindingItems = oTable.getBinding("items");
            var aFilters=[];
            oBindingItems.filter(aFilters);
        ...
        _onObjectMatched: function (oEvent) {
            ...
            this.getModel().metadataLoaded().then(function () {
                var sObjectPath = this.getModel().createKey("CarrierSet", {
                    Carrid: sObjectId
                });
